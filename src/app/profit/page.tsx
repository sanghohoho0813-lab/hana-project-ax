...
export default function ProfitPage() {
  ...
  return (
    <div className="page-in space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={k.label} className="card p-4.5">
            ...
            <p
              className={`mt-1 text-[19px] font-extrabold tracking-tight ${k.accent ? "text-success" : ""}`}
            >
              {k.value}
            </p>
            {k.sub && <p className="mt-1 text-[11.5px] text-danger">{k.sub}</p>}
          </div>
        ))}
      </div>
      ...
      {focus && (
        <section className="card p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-[16.5px] font-bold">{focus.name}</h3>
            ...
            <select
              ...
              className="rounded-xl border border-line bg-white px-3 py-2 text-[13px] font-semibold text-ink-2 outline-none focus:border-primary"
            >
              {active.map((p) => (
          ...
          {isCostRisk(focus) ? (
            <div className="mt-4 rounded-2xl border border-danger/15 bg-danger-bg/50 p-5">
              ...
              <div className="mt-3.5 space-y-1.5">
                {overruns.map((o) => (
                  <div
                    key={o.name}
                    className="flex items-center justify-between rounded-xl bg-white px-4 py-2.5 text-[13px]"
                  >
            ...
            <p className="mt-4 rounded-2xl bg-success-bg/50 px-5 py-4 text-[13.5px] font-semibold text-success">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl bg-[#f7f8fa] p-5">
              <p className="mb-3 text-[13.5px] font-bold">이익 변화</p>
            ...
            <div className="rounded-2xl bg-[#f7f8fa] p-5">
              <p className="mb-3 text-[13.5px] font-bold">감소 원인</p>
              ...
              >
                항목별 원가 자세히 보기 <ChevronRight size={13} />
              </Link>
            </div>
          </div>
        </section>
      ...
      {riskProjects.length > 0 && (
        <section>
          <h3 className="mb-3 flex items-center gap-2 text-[17px] font-bold">
            <TrendingDown size={17} className="text-danger" /> 수익성 위험 {riskProjects.length}건
          </h3>
      ...
      {/* 프로젝트별 현황 */}
      <section>
        <h3 className="mb-3 text-[17px] font-bold">프로젝트별 원가 현황</h3>
        <div className="space-y-2.5">
          {active.map((p) => {
      ...
      <section className="card p-5">
        <p className="text-[14.5px] font-bold">공사별 예상이익 비교</p>
