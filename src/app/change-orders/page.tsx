      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={k.label} className="card p-4.5">
            ...
            <p className={`mt-1 text-[20px] font-extrabold ${"danger" in k && k.danger ? "text-danger" : ""}`}>
              {k.value}
            </p>
          </div>
        ))}
      ...
      {verbal.length > 0 && (
        <div className="card pulse-danger border border-danger/15 bg-danger-bg/40 p-5">
          <p className="flex items-center gap-2 text-[14px] font-bold text-danger">
            <AlertTriangle size={16} /> 서면승인이 필요한 요청이 있습니다
          </p>
          {verbal.map((c) => (
                ...
                <div className="mt-3.5 flex flex-wrap items-end justify-between gap-3">
                  <div className="grid grid-cols-3 gap-4 text-[13px]">
                    <div>
                      ...
                      <p className="text-[15px] font-extrabold">{formatMoney(c.addRevenue)}</p>
                    </div>
                    <div>
                      <p className="text-ink-3">추가원가</p>
                      <p className="text-[15px] font-extrabold">{formatMoney(c.addCost)}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {next && (
                      <button
                        ...
                        className="rounded-xl bg-primary-light px-3.5 py-2 text-[12.5px] font-bold text-primary-dark transition-colors hover:bg-[#dcebfd]"
                      >
                        다음 단계: {next}
                      </button>
                    )}
