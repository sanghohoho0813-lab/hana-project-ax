            ...
            {targets.map((c) => (
              <div key={c.id} className="card card-hover p-5">
                ...
                <div className="mt-3 rounded-2xl bg-[#f7f8fa] p-4">
                  <p className="text-[11.5px] font-semibold text-ink-3">추천 이유</p>
                  <ul className="mt-1 space-y-0.5">
                    {c.signals.map((s) => (
                      <li key={s} className="flex gap-1.5 text-[13px] text-ink-2">
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 text-[13px]">
                  <div>
                    <p className="text-[11.5px] text-ink-3">제안 서비스</p>
                    <p className="font-bold">{c.recommend}</p>
                  </div>
                  <div>
                    <p className="text-[11.5px] text-ink-3">예상 제안금액</p>
                    <p className="font-extrabold">{formatMoney(c.proposalAmount)}</p>
                  </div>
                  <div>
                    <p className="text-[11.5px] text-ink-3">연락 추천일</p>
                    <p className="font-bold">{formatDateFull(c.nextProposal)}</p>
                  </div>
                </div>
                <p className="mt-2 text-[12px] text-ink-3">담당 {c.manager}</p>
          ...
          {customers.map((c) => (
            <div key={c.id} className="card card-hover p-5">
              ...
              <p className="flex items-center gap-1 text-[12.5px] text-ink-3">
                <MapPin size={12} /> {c.region} · 최근 공사 {formatDateFull(c.lastWorkDate)}
              </p>
              <div className="mt-3.5 grid grid-cols-2 gap-y-2.5 text-[13px]">
                <div>
                  <p className="text-[11.5px] text-ink-3">누적 계약금액</p>
                  <p className="font-extrabold">{formatMoney(c.totalAmount)}</p>
                </div>
                <div>
                  <p className="text-[11.5px] text-ink-3">미수금</p>
                  <p className={`font-extrabold ${c.receivable > 0 ? "text-danger" : ""}`}>
                    {c.receivable > 0 ? formatMoney(c.receivable) : "없음"}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className="text-[11.5px] text-ink-3">진행한 공사</p>
        ...
        {draft && (
          <div>
            ...
            <div className="mt-5 space-y-4 text-[14px] leading-relaxed">
              ...
              {draft.receivable > 0 && (
                <p className="rounded-xl bg-warning-bg px-4 py-3 text-[13px] font-semibold text-warning">
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                ...
                className="rounded-xl bg-primary px-4 py-2.5 text-[14px] font-semibold text-white transition-colors hover:bg-primary-dark"
              >
                문서함에 저장
              </button>
            </div>
